...
//	}
	...
	public String dragBoardString(){
		return token.getType() + " " + token.getImage() + " " + index;
	}
	...
	protected ImageView initImageView(Image img){
		...
		if(token.getType().equals("removed")){
...
//	                            image.getHeight() /2));
				...
				public void handle(DragEvent event) {
					...
					if(!occupied){
						...
						if (db.hasString()) {
							...
							LinkedList<LHSIconizedToken> iTokens = 
	...
	public ImageView occupyEmptySpace(final ImageView imgView){
		...
		imgView.setOnDragDone(new EventHandler<DragEvent>() {
			...
			public void handle(DragEvent event) {
				LinkedList<LHSIconizedToken> iTokens = GameBoard.getInstance().getTokenBoardItokens();
				if (event.getTransferMode() == TransferMode.MOVE) {

					LHSIconizedToken element = iTokens.remove(index);
					iTokens.add(index, new LHSIconizedToken(element.getImage(), new SourceToken("removed", "removed"), index));
